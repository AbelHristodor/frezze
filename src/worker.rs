...
impl FreezeSchedulerWorker {
    ...
    async fn check_and_activate_scheduled_freezes(&self) -> anyhow::Result<()> {
        ...
        info!("Found {} scheduled freezes to activate", scheduled_freezes.len());

        for freeze_record in scheduled_freezes {
            match self.activate_scheduled_freeze(&freeze_record).await {
                Ok(_) => {
                    info!("Successfully activated scheduled freeze for repository: {}", freeze_record.repository);
                }
                Err(e) => {
                    error!("Failed to activate scheduled freeze for repository {}: {}", freeze_record.repository, e);
                }
            }
        }
    ...
    async fn get_scheduled_freezes_to_activate(&self, conn: &sqlx::PgPool) -> anyhow::Result<Vec<FreezeRecord>> {
        let now = Utc::now();
    ...
    async fn activate_scheduled_freeze(&self, freeze_record: &FreezeRecord) -> anyhow::Result<()> {
        ...
        if parts.len() != 2 {
            return Err(anyhow::anyhow!("Invalid repository format: {}", freeze_record.repository));
        }
        ...
        match self.apply_freeze_to_repository(&freeze_manager, freeze_record.installation_id as u64, &repository).await {
            Ok(_) => {
                info!("Successfully applied freeze to repository: {}", freeze_record.repository);
            }
            Err(e) => {
                warn!("Failed to apply freeze operations to repository {}: {}. Freeze record is still marked as active.", 
    ...
    async fn apply_freeze_to_repository(
        ...
        freeze_manager
            ...
            .await
            .map_err(|e| anyhow::anyhow!("Failed to refresh PRs for repository {}: {}", repository.full_name(), e))?;

        info!("Successfully applied freeze operations to repository: {}", repository.full_name());
        Ok(())
    }
}